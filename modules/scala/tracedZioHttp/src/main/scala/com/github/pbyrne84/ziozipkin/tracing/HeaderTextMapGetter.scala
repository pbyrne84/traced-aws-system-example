...
class HeaderTextMapGetter extends TextMapGetter[List[Header]] with StrictLogging {
  ...
  override def get(carrier: List[Header], key: String): String = {
    logger.info(s"get key '$key' from ${carrier.map(_.headerName)}")
    val maybeValue = carrier.find(_.headerName.toLowerCase == key.toLowerCase).map(_.renderedValue).orNull
